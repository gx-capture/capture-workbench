...
test('release manifest is immutable, candidate-bound, and records registry and gate evidence', async () => {
  ...
  try {
    ...
    const snapshot = Buffer.from(
      `${JSON.stringify({ schemaVersion: '1', releaseVersion: '0.4.2' })}\n`,
    );
    await writeFile(
    ...
    await writeFile(
      ...
      JSON.stringify({
        ...
        artifacts: [
          {
            name: 'capture_runtime_client-0.4.2-py3-none-any.whl',
            sha256: 'e'.repeat(64),
          },
        ],
      }),
    ...
    await writeFile(
      ...
      JSON.stringify({
        ...
        artifacts: [
          {
            name: 'capture-sidecar-launcher-0.4.2.crate',
    ...
    await writeFile(
      ...
      JSON.stringify({
        ...
        artifacts: [
          {
            name: 'capture-runtime-client-0.4.2.jar',
            sha256: 'f'.repeat(64),
          },
        ],
      }),
