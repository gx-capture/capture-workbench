...
export const REAL_MODEL_RELEASE_VERSION = '0.4.2';
...
function validateSourceLockAndCatalog(
  ...
  if (lock.lockVersion !== '2') {
    throw new Error('Model source lock schema must be v2 for the 0.4.2 gate.');
  }
  if (lock.releaseVersion !== REAL_MODEL_RELEASE_VERSION) {
    throw new Error('Model source lock release version is not 0.4.2.');
  }
  ...
  if (catalogObject.runtimeVersion !== REAL_MODEL_RELEASE_VERSION) {
    throw new Error('Generated model catalog runtime version is not 0.4.2.');
  }
