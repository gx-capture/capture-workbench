"""Token budgets and provider response schemas for runtime structuring."""
...
"""JSON Schema for identity semantic block batches."""


def structuring_batch_schema(*, target_language: str | None) -> dict[str, object]:
    """Return the canonical semantic response schema for one operation."""
...
def _ollama_generation_schema(schema: Mapping[str, object]) -> dict[str, object]:
    """Remove grammar-hostile text maxima while retaining validation semantics."""
...
"""Ollama-compatible schema for identity semantic block batches."""


def ollama_structuring_batch_schema(*, target_language: str | None) -> dict[str, object]:
    """Return the grammar-compatible schema for the requested operation."""
