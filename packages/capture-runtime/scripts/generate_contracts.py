...
def _typescript_source(models: dict[str, dict[str, Any]], enums: dict[str, list[str]]) -> bytes:
    lines = [
...
def _manifest(models: dict[str, dict[str, Any]]) -> dict[str, Any]:
    ...
    return {
        ...
        "aliases": [{"name": "CaptureLocatorV1", "type": "PageLocatorV1 | TimeLocatorV1"}],
...
def _expected_files(output: Path) -> dict[Path, bytes]:
    ...
    files: dict[Path, bytes] = {
        output / "src" / "generated" / "contracts.ts": _typescript_source(models, _enum_values()),
