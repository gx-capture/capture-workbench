...
def test_catalog_generation_rejects_first_party_git_blob_drift(
    tmp_path: Path,
) -> None:
    engine_dir = tmp_path / "engines"
    engine_dir.mkdir()
    ...
    provenance = next(
        item
        for item in source_lock["requirements"][0]["files"]
    ...
    assert "first-party Git blob" in result.stderr
